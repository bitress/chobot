"""Legacy regex-based intent extractor.

This module provides the original standalone regex `extract_search_intent`
classifier. It is kept as a fallback for when no LLM API key is configured.

For the primary LLM-based path, see intent_extractor.py.
"""
import re
...
def extract_search_intent(question: str) -> SearchIntent:
    ...
    Returns a dict with keys:
      - intent: 'item', 'villager', 'island', or 'none'
    ...
    # Island characteristic patterns (e.g., "which island has the zelda theme?")
    island_patterns = [
        (r"^(?:which|what)\s+island\s+(?:has|has got|is)\s+(?:the\s+)?(.+?)(?:\s+theme|themed)?$", "island"),
        (r"^which\s+island\s+(?:has|is)\s+(.+)$", "island"),
        (r"^(?:find|search)\s+(?:the\s+)?island\s+(?:with|by)\s+(.+)$", "island"),
    ]
    for pattern, kind in island_patterns:
        match = re.match(pattern, q, flags=re.IGNORECASE)
        if match:
            query = _canonical_query(match.group(1))
